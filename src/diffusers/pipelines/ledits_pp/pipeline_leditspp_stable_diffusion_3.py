...
class LEditsPPPipelineStableDiffusion3(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    r"""
    Args:
    ...
    def encode_prompt(
        ...
        if editing_prompt is not None:
            ...
            for single_prompt in editing_prompt:
                prompt_2 = single_prompt
                prompt_3 = single_prompt

                edit_prompt_embed, edit_pooled_prompt_embed = self._get_clip_prompt_embeds(
                    single_prompt,
                    device=device,
                ...
                edit_prompt_2_embed, edit_pooled_prompt_2_embed = self._get_clip_prompt_embeds(
                    prompt_2,
                    device=device,
                    num_images_per_prompt=num_images_per_prompt,
                    clip_skip=None,
                ...
                edit_clip_prompt_embeds = torch.cat([edit_prompt_embed, edit_prompt_2_embed], dim=-1)

                t5_edit_prompt_embed = self._get_t5_prompt_embeds(
                    prompt=prompt_3,
                    num_images_per_prompt=num_images_per_prompt,
                ...
                edit_clip_prompt_embeds = torch.nn.functional.pad(
                    edit_clip_prompt_embeds,
                    (0, t5_edit_prompt_embed.shape[-1] - edit_clip_prompt_embeds.shape[-1]),
                )

                single_edit_prompt_embeds = torch.cat([edit_clip_prompt_embeds, t5_edit_prompt_embed], dim=-2)
                single_edit_pooled_prompt_embeds = torch.cat(
                    [edit_pooled_prompt_embed, edit_pooled_prompt_2_embed], dim=-1
                )
                editing_prompt_embeds.append(single_edit_prompt_embeds)
                editing_pooled_prompt_embeds.append(single_edit_pooled_prompt_embeds)
            # Convert lists to tensors
            editing_prompt_embeds = torch.cat(editing_prompt_embeds)
            editing_pooled_prompt_embeds = torch.cat(editing_pooled_prompt_embeds)
        ...
        return negative_prompt_embeds,  negative_pooled_prompt_embeds, editing_prompt_embeds, editing_pooled_prompt_embeds
    ...
    def __call__(
        ...
        skip_noise_for_edit: bool = False,
    ):
        r"""
        ...
        if enable_edit_guidance:
            ...
            pooled_prompt_embeds = torch.cat([pooled_prompt_embeds, pooled_edit_embeds], dim=0)
        ...
        with self.progress_bar(total=len(timesteps)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latents] * (1 + enabled_editing_prompts))
                ...
                # perform guidance
                    ...
                    for c, noise_pred_edit_concept in enumerate(noise_pred_edit_concepts):
                        ...
                        if True:#elif not use_cross_attn_mask:
                            # calculate quantile
                            noise_guidance_edit_tmp_quantile = torch.abs(noise_guidance_edit_tmp)
                            noise_guidance_edit_tmp_quantile = torch.sum(
                                noise_guidance_edit_tmp_quantile, dim=1, keepdim=True
                            )
                            noise_guidance_edit_tmp_quantile = noise_guidance_edit_tmp_quantile.repeat(1, 16, 1, 1)
                ...
                idx = t_to_idx[int(t)]
                #print(timestep, idx, zs[idx][0, 2, 4:9, 8:10].mean())
                latents = self.scheduler.step(noise_pred, t, latents, return_dict=False, s_churn=1.0, eps=zs[idx])[0]
    ...
    def encode_image(self, image, dtype=None, height=None, width=None, resize_mode="default", crops_coords=None):
        ...
        image = image.to(self._execution_device, dtype=dtype)
    ...
    def invert(
        ...
        #um_warmup_steps = max(len(timesteps) - num_inference_steps * self.scheduler.order, 0)
        #self._num_timesteps = len(timesteps)

        # autoencoder reconstruction

        image_rec = \
            self.vae.decode(x0 / self.vae.config.scaling_factor, return_dict=False, generator=generator)[0]
        ...
        t_to_idx = {int(v): k for k, v in enumerate(timesteps)}
        xts = torch.zeros(size=variance_noise_shape, device=self._execution_device, dtype=prompt_embeds.dtype)

        for t in reversed(timesteps):
            idx = num_inversion_steps - t_to_idx[int(t)] - 1
            noise = randn_tensor(shape=x0.shape, generator=generator, device=self._execution_device, dtype=x0.dtype)
            xts[idx] = self.scheduler.add_noise(x0, noise, t.unsqueeze(0))
        xts = torch.cat([x0.unsqueeze(0), xts], dim=0)

        # noise maps
        zs = torch.zeros(size=variance_noise_shape, device=self._execution_device, dtype=prompt_embeds.dtype)
        ...
        for t in self.progress_bar(timesteps):
            ...
            # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
            timestep = t.expand(latent_model_input.shape[0]).to(self._execution_device)
            ...
            xtm1 = xts[idx]
            z, xtm1_corrected = compute_noise_recflow(self.scheduler, xtm1, xt, t, noise_pred)
            #print(timestep, idx, z[0, 2, 4:9, 8:10].mean(), xtm1_corrected[0, 2, 4:9, 8:10].mean())
            zs[idx] = z
...
def compute_noise_recflow(scheduler, prev_latents, latents, timestep, noise_pred):
    s_churn = 1.0
    ...
    dt = scheduler.sigmas[step_index + 1] - sigma_hat
    derivative = noise_pred * sigma / sigma_hat

    mu_xt = sample + (dt * derivative)

    noise = (prev_latents - mu_xt) / ((sigma_hat**2 - sigma**2) ** 0.5)
    # Return computed noise and error corrected prediction
    corrected = mu_xt + noise * ((sigma_hat ** 2 - sigma ** 2) ** 0.5)
    #print(timestep, sigma, gamma, noise[0, 2, 4:9, 8:10].mean(), corrected[0, 2, 4:9, 8:10].mean())

    return noise, corrected